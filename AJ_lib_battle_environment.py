...
class env:
    ...
    def set_up(self):
        ...
        observation = self.ball.y, self.ball.x, self.pad1.y, self.pad2.y
        return np.array(observation)/self.size

    def step(self, action1, action2):
        self.pad1.action(action1)
        self.pad2.action(action2)

        self.pad1.boundaries()
        self.pad2.boundaries()
        self.ball.boundaries()

        is_hit1 = self.ball.hit(self.pad1)
        is_hit2 = self.ball.hit(self.pad2)
        ...
        def decide_rew(other, is_hit, done, lose_side, numpad):
            reward = self.reward_still_alive
            if self.ball.y < other.y + other.y_size and self.ball.y > other.y - other.y_size:
        ...
        reward1 = decide_rew(self.pad1, is_hit1, done, lose_side, 1)
        reward2 = decide_rew(self.pad2, is_hit2, done, lose_side, 2)
